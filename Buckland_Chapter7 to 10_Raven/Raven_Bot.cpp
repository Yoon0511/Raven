...
bool Raven_Bot::HandleMessage(const Telegram& msg)
{
  ...
  case Msg_TakeThatMF:
  {
      ...
      int dg = DereferenceToType<int>(msg.ExtraInfo);
      ...
      if (isDead())
      {
          ...
          // 죽었을 시
          // 다른 봇들의 MemoryRecord에서 누적 데미지값을 초기화 한다.
          ...
          {
              (*curBot)->m_pSensoryMem->UpdateOtherBotHitDamage(ID(), 0, true);
          }
      }
      else
      {
    ...
    // 다른 봇이 공격당한 것을 메시지 받음
  case Msg_OtherBotsHit:
  {
      int dg = DereferenceToType<int>(msg.ExtraInfo);
      int hitbotID = msg.Sender;
      m_pSensoryMem->UpdateOtherBotHitDamage(hitbotID, dg,false);
  }
      return true;
