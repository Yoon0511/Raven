...
void Raven_SensoryMemory::UpdateOtherBotHitDamage(const int hitbotID, const int damage,bool isDead)
{
    int myID = m_pOwner->ID();
    int HitBotId = hitbotID;
    int Damage = damage;
    // 피격 당한 봇이 자신의 시야각 안에 있으면 누적데미지값을 증가 시켜준다.
    UpdateVision();
    ...
    for (curBot = bots.begin(); curBot != bots.end(); ++curBot)
    {
        if (m_pOwner != *curBot)
        {
            if ((*curBot)->ID() == HitBotId) // 맞은 애일 경우
            {
                MakeNewRecordIfNotAlreadyPresent(*curBot);

                MemoryRecord& info = m_MemoryMap[*curBot];

                if (isDead) //죽은 봇이라면 누적 데미지를 초기화 시킨다.
                {
                    info.Sumdamage = 0;
                    continue;
                }
                if (info.bWithinFOV)
                {
                    info.Sumdamage += Damage;
                }
                //#ifdef LOG_CREATIONAL_STUFF
                //debug_con << "hit damage" << std::to_string(info.Sumdamage) << "";
                //#endif
            }
        }
    }
...
int Raven_SensoryMemory::GetSumDamage(int id)
{
    const std::list<Raven_Bot*>& bots = m_pOwner->GetWorld()->GetAllBots();
    std::list<Raven_Bot*>::const_iterator curBot;
    for (curBot = bots.begin(); curBot != bots.end(); ++curBot)
    {
        if (m_pOwner != *curBot)
        {
            if ((*curBot)->ID() == id) // 맞은 애일 경우
            {
                MakeNewRecordIfNotAlreadyPresent(*curBot);

                MemoryRecord& info = m_MemoryMap[*curBot];

                return info.Sumdamage;
            }
        }
    }
    return -1;
